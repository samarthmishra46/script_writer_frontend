...
const ImageSwiper: React.FC = () => {
  ...
  const fetchAd = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl(`api/ads/${state.adId}`), {
        headers: {
  ...
  const handleSwipe = async (image: GeneratedImage, direction: 'left' | 'right') => {
    try {
      ...
      await fetch(buildApiUrl(`api/ads/${state?.adId}/swipe`), {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
            ...
            >
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent p-6">
                  <h3 className="text-white text-2xl font-bold mb-1">{image.variationName}</h3>
                  <p className="text-gray-300 text-sm capitalize">
                    {image.variationType.replace(/-/g, ' ')}
                  </p>
                </div>
              </div>
            </TinderCard>
          ...
          {savedImages.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-sm mt-2">
                Click the heart icon on images you like to save them
              </p>
            </div>
          ) : (
              ...
              {savedImages.map((image) => (
                <div key={image._id} className="relative group rounded-xl overflow-hidden bg-gray-800">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3">
                    <button
                      onClick={() => onDownload(image.imageUrl, `${brandName}-${image.variationName}.png`)}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                    <p className="text-white text-sm font-medium truncate">{image.variationName}</p>
            ...
            <button
              onClick={() => {
                savedImages.forEach((img, i) => {
                  setTimeout(() => {
                    onDownload(img.imageUrl, `${brandName}-${img.variationName}-${i + 1}.png`);
                  }, i * 500);
                });
              }}
